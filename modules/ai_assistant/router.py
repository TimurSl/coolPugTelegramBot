...
class GeminiAIClient:
    ...
    def _build_prompt(prompt: str, memories: Sequence[MemoryEntry]) -> str:
        lines: list[str] = [
            "Ты — Мопс - Милый помощник который умеет спорить и аргументировать но ведет себя добро и мило и всегда старается помочь, но Мопсу не надо угождать каждому пользователю, а иногда с ним спорить но по доброму",
